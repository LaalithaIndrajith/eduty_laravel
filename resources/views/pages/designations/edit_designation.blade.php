...
@section('content')
    ...
    {{-- Create Designation--}}
    <form method="POST" id="edit-designation-form" action="{{ route('editDesignation', $designation->designation_id) }}" enctype="multipart/form-data">
        @csrf
        <div id="kt_page_sticky_card" class="card card-custom card-sticky">
            <div class="card-header flex-wrap py-3">
                <div class="card-title">
                    <h3 class="card-label">Edit Designation Details
                    <span class="d-block text-muted pt-2 font-size-sm">Edi Designation details for the system </span></h3>
                </div> 
                    ...
                    <div class="row d-flex justify-content-around"> 
                        <div class="col-lg-3 form-group">
                            ...
                            <select class="form-control form-control-lg dynamic mt-2 selectpicker @error('department_select') is-invalid @enderror" name="department_select" id="department_select" data-dependent="department_select" data-size="7" data-live-search="true">
                                ...
                                <option value="{{ $department->depart_id }}" {{ ($designation->depart_id == $department->depart_id) ? 'selected' : '' }}>{{ $department->depart_code }} | {{ $department->depart_name }}</option>   
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-3 form-group">
                            ...
                            <input id="desig_code" type="text" class="form-control @error('desig_code') is-invalid @enderror" name="desig_code" value="{{ $designation->designation_code }}" />
                            @error('desig_code')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="col-lg-3 form-group">
                            ...
                            <input id="desig_name" type="text" class="form-control @error('desig_name') is-invalid @enderror" name="desig_name" value="{{ $designation->designation_name }}" />
                            @error('desig_name')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
...
@section('scripts')
    ...
    <script>
        ...
        document.getElementById('edit-designation-form'), {
                fields: {
                    department_select: {
                        validators: {
                            notEmpty: {
        ...
        let designationEditEvent = <?php 
            if(session()->has('designationEdit')){
                echo json_encode(session()->get('designationEdit'));   
            }else{
                echo json_encode('');
            } 
            ?>;

        (designationEditEvent != '') ? loadToaster(designationEditEvent) : ''
